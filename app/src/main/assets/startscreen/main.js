...
CFG_CD="";
CFG_CONFIG="release";
CFG_GLFW_WINDOW_HEIGHT="150";
CFG_GLFW_WINDOW_WIDTH="200";
CFG_HOST="linux";
...
//${METADATA_BEGIN}
var META_DATA="[mojo_font.png];type=image/png;width=864;height=13;\n[eyeani.png];type=image/png;width=1800;height=150;\n";
//${METADATA_END}
...
function c_MainApp(){
	c_App.call(this);
	this.m_img=null;
	this.m_aniStartTime=0;
	this.m_mode=1;
	this.m_counter=0.0;
	this.m_aniLen=0.0;
	this.m_IX=0;
	this.m_IY=0;
	this.m_mainCalled=false;
}
...
c_MainApp.prototype.p_StartAni=function(t_aid){
	this.m_aniStartTime=bb_app_Millisecs();
	this.m_mode=t_aid;
	this.m_counter=0.0;
	var t_1=t_aid;
	if(t_1==1){
...
c_MainApp.prototype.p_OnCreate=function(){
	this.m_img=bb_graphics_LoadImage("eyeani.png",9,c_Image.m_DefaultFlags);
	this.m_img.p_SetHandle(100.0,75.0);
	this.p_StartAni(0);
	bb_app_SetUpdateRate(60);
	return 0;
}
c_MainApp.prototype.p_OnUpdate=function(){
	this.m_IX=(((bb_app_DeviceWidth())*0.5)|0);
	this.m_IY=(((bb_app_DeviceHeight())*0.3)|0);
	print(String(bb_app_DeviceWidth())+"x"+String(bb_app_DeviceHeight()));
	if((bb_input_KeyHit(27))!=0){
		bb_app_EndApp();
	}
	if(!this.m_mainCalled && bb_app_Millisecs()>500 && ((bb_input_MouseDown(0))!=0)){
		ShowMainMenu();
		this.m_mainCalled=true;
	}
	if(!this.m_mainCalled && bb_app_Millisecs()>3000){
		ShowMainMenu();
		this.m_mainCalled=true;
	}
	return 0;
}
c_MainApp.prototype.p_RenderAni=function(){
	bb_graphics_Cls(255.0,255.0,255.0);
	var t_p=(bb_app_Millisecs()-this.m_aniStartTime)/this.m_aniLen;
	if(this.m_mode!=5){
		if(t_p>=1.0){
			this.p_StartAni(this.m_mode+1);
			t_p=0.01;
		}
	}else{
		t_p=0.01;
	...
	var t_p2=.0;
	var t_s=1.0;
	if(bb_app_Millisecs()<1700){
		t_s=0.8+bb_app2_EaseOutQuad((bb_app_Millisecs())/1700.0)*0.2;
	}
	var t_2=this.m_mode;
	if(t_2==0){
		bb_graphics_SetAlpha(t_p);
		bb_graphics_DrawImage2(this.m_img,(this.m_IX),(this.m_IY),0.0,t_s,t_s,8);
		bb_graphics_SetAlpha(1.0);
	}else{
		if(t_2==1 || t_2==4){
			t_x=((9.0-t_p*9.0)|0);
			t_x=bb_math_Clamp(t_x,0,8);
			bb_graphics_DrawImage2(this.m_img,(this.m_IX),(this.m_IY),0.0,t_s,t_s,t_x);
			t_x2=t_x+1;
			t_x2=bb_math_Clamp(t_x2,0,8);
			t_p2=9.0-t_p*9.0-((9.0-t_p*9.0)|0);
			bb_graphics_SetAlpha(t_p2);
			bb_graphics_DrawImage2(this.m_img,(this.m_IX),(this.m_IY),0.0,t_s,t_s,t_x2);
			bb_graphics_SetAlpha(1.0);
		}else{
			if(t_2==3){
				t_x=9-((9.0-t_p*9.0)|0);
				...
				t_x2=t_x+1;
				t_x2=bb_math_Clamp(t_x2,0,8);
				t_p2=1.0-(9.0-t_p*9.0-((9.0-t_p*9.0)|0));
				bb_graphics_SetAlpha(t_p2);
				bb_graphics_DrawImage2(this.m_img,(this.m_IX),(this.m_IY),0.0,t_s,t_s,t_x2);
				bb_graphics_SetAlpha(1.0);
			}else{
				if(t_2==5 || t_2==2){
					bb_graphics_DrawImage2(this.m_img,(this.m_IX),(this.m_IY),0.0,t_s,t_s,0);
				}
			}
		}
	}
...
function bb_app2_EaseOutQuad(t_delta){
	if(t_delta<0.0){
		return 0.0;
	}
	if(t_delta>1.0){
	...
	return 1.0-(1.0-t_delta)*(1.0-t_delta);
}
