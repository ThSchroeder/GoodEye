...
//${METADATA_BEGIN}
var META_DATA="[mojo_font.png];type=image/png;width=864;height=13;\n[1.jpg];type=image/jpg;width=800;height=600;\n[2.jpg];type=image/jpg;width=800;height=600;\n[3.jpg];type=image/jpg;width=800;height=600;\n[4.jpg];type=image/jpg;width=800;height=600;\n[5.jpg];type=image/jpg;width=800;height=600;\n[6.jpg];type=image/jpg;width=800;height=600;\n[7.jpg];type=image/jpg;width=800;height=600;\n[8.jpg];type=image/jpg;width=800;height=600;\n[9.jpg];type=image/jpg;width=800;height=600;\n";
//${METADATA_END}
...
function c_MainApp(){
	c_App.call(this);
	this.m_images=[];
	this.m_sliceLength=.0;
	this.m_sl=null;
	this.m_currentSlice=0;
	this.m_inBetween=.0;
	this.m_lastValue=0.0;
	this.m_change=5;
	this.m_lastWidth=0.0;
...
c_MainApp.prototype.p_OnCreate=function(){
	var t_imageNames=string_trim(bb_app_LoadString("imginfo.txt")).split(";");
	this.m_images=new_object_array(t_imageNames.length);
	for(var t_i=0;t_i<t_imageNames.length;t_i=t_i+1){
		this.m_images[t_i]=bb_graphics_LoadImage(t_imageNames[t_i],1,c_Image.m_DefaultFlags);
	}
	this.m_sliceLength=1.0/((t_imageNames.length)-1.0);
	this.m_sl=c_Slider.m_new.call(new c_Slider);
	this.m_sl.m_value=0.5;
	this.m_sl.p_SetSliderColor(0,100,200);
	bb_app_SetUpdateRate(25);
	return 0;
}
c_MainApp.prototype.p_OnUpdate=function(){
	...
	this.m_sl.p_Update();
	this.m_currentSlice=((this.m_sl.m_value/this.m_sliceLength)|0);
	this.m_inBetween=this.m_sl.m_value/this.m_sliceLength-(this.m_currentSlice);
	if(this.m_sl.m_value!=this.m_lastValue){
		this.m_lastValue=this.m_sl.m_value;
		this.m_change=5;
	}
	this.m_change=5;
	return 0;
...
c_MainApp.prototype.p_OnRender=function(){
	if((bb_app_DeviceWidth())!=this.m_lastWidth || (bb_app_DeviceHeight())!=this.m_lastHeight){
		this.m_lastWidth=(bb_app_DeviceWidth());
		this.m_lastHeight=(bb_app_DeviceHeight());
		this.m_change=5;
	}
	if(this.m_change<=0){
		return 0;
	...
	var t_sc=(bb_app_DeviceWidth())/(this.m_images[this.m_currentSlice].p_Width());
	var t_iy=((bb_app_DeviceHeight())-(this.m_images[this.m_currentSlice].p_Height())*t_sc)/2.0;
	if(this.m_currentSlice==this.m_images.length-1){
		bb_graphics_SetAlpha(1.0);
		bb_graphics_DrawImage2(this.m_images[this.m_currentSlice],0.0,t_iy,0.0,t_sc,t_sc,0);
	}else{
		bb_graphics_SetAlpha(1.0);
		bb_graphics_DrawImage2(this.m_images[this.m_currentSlice],0.0,t_iy,0.0,t_sc,t_sc,0);
		bb_graphics_SetAlpha(this.m_inBetween);
		bb_graphics_DrawImage2(this.m_images[this.m_currentSlice+1],0.0,t_iy,0.0,t_sc,t_sc,0);
	}
	bb_graphics_SetAlpha(1.0);
	this.m_sl.p_SetShape(30.0,(bb_app_DeviceHeight()-50),(bb_app_DeviceWidth()-60),40.0);
	this.m_sl.p_Render();
	return 0;
}
